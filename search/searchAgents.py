...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if all(visitedCorners):
        return 0

    distances = [
        util.manhattanDistance(currentPosition, corner) 
        for corner, visited in zip(corners, visitedCorners) if not visited
    ]
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    farthest_food = max(map(lambda xy: mazeDistance(xy, position, problem.startingGameState), foodList))
